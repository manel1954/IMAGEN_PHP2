...
function formatFreq($hz) { $mhz = intval($hz)/1000000; return number_format($mhz,3,'.',''). ' MHz'; }

if ($action === 'station-info') {
    $ini = parseMMDVMIni('/home/pi/MMDVMHost/MMDVMHost.ini');
    $callsign = $ini['General']['Callsign'] ?? 'EA3EIZ';
    $txfreq   = $ini['General']['TXFrequency'] ?? ($ini['General']['Frequency'] ?? '430000000');
    $lat = $ini['Info']['Latitude'] ?? '41.3851'; $lon = $ini['Info']['Longitude'] ?? '2.1734';
    $location = $ini['Info']['Location'] ?? 'Barcelona'; $desc = $ini['Info']['Description'] ?? '';
    $locator = (floatval($lat)!=0||floatval($lon)!=0) ? latLonToLocator($lat,$lon) : 'JN11CK';
    $port = $ini['Modem']['UARTPort'] ?? ($ini['modem']['UARTPort'] ?? '');
    $rxhz=$ini['Info']['RXFrequency']??'0'; $txhz=$ini['Info']['TXFrequency']??$txfreq;
    ...
    $ip=$iniIp?:'—';
    $ysfIni=parseMMDVMIni('/home/pi/MMDVMHost/MMDVMYSF.ini');
    $ysfPort=$ysfIni['Modem']['UARTPort']??($ysfIni['modem']['UARTPort']??'—');
    $ysfFreqRX=formatFreq($ysfIni['Info']['RXFrequency']??'0');
    $ysfFreqTX=formatFreq($ysfIni['Info']['TXFrequency']??'0');
    $ysfIpRaw=trim($ysfIni['General']['Address']??'');
    $ysfIp=($ysfIpRaw!==''&&$ysfIpRaw!=='0.0.0.0')?$ysfIpRaw:$ip;
    header('Content-Type: application/json');
    echo json_encode(['callsign'=>strtoupper(trim($callsign)),'freq'=>$freq,'freqRX'=>$freqRX,'port'=>$port?:'—','ip'=>$ip,'locator'=>$locator,'location'=>trim($location),'desc'=>trim($desc),'lat'=>$lat,'lon'=>$lon,'ysfPort'=>$ysfPort?:'—','ysfFreqRX'=>$ysfFreqRX,'ysfFreqTX'=>$ysfFreqTX,'ysfIp'=>$ysfIp?:'—']);
    exit;
}
...
if ($action === 'sysinfo') {
    ...
    $dTotal=array_sum(array_slice($cpu2,1))-array_sum(array_slice($cpu1,1));
    $dIdle=$cpu2[4]-$cpu1[4];
    ...
    $ramTotal=round($mem['MemTotal']/1048576,2); $ramFree=round(($mem['MemAvailable']??$mem['MemFree'])/1048576,2); $ramUsed=round($ramTotal-$ramFree,2);
    $diskTotal=round(disk_total_space('/')/1073741824,1); $diskFree=round(disk_free_space('/')/1073741824,1); $diskUsed=round($diskTotal-$diskFree,1);
    $temp=''; if(file_exists('/sys/class/thermal/thermal_zone0/temp')) $temp=round(intval(trim(file_get_contents('/sys/class/thermal/thermal_zone0/temp')))/1000,1).' °C';
    header('Content-Type: application/json');
...
if ($action === 'status') {
    header('Content-Type: application/json');
    echo json_encode(['gateway'=>trim(shell_exec('systemctl is-active dmrgateway 2>/dev/null')),'mmdvm'=>trim(shell_exec('systemctl is-active mmdvmhost 2>/dev/null'))]);
    exit;
}
if ($action === 'start') { saveState('dmr','on'); shell_exec('sudo systemctl start dmrgateway 2>/dev/null'); sleep(2); shell_exec('sudo systemctl start mmdvmhost 2>/dev/null'); header('Content-Type: application/json'); echo json_encode(['ok'=>true]); exit; }
if ($action === 'stop')  { saveState('dmr','off'); shell_exec('sudo systemctl stop mmdvmhost 2>/dev/null'); sleep(1); shell_exec('sudo systemctl stop dmrgateway 2>/dev/null'); header('Content-Type: application/json'); echo json_encode(['ok'=>true]); exit; }

if ($action === 'update-imagen') { $o=shell_exec('sudo sh /home/pi/A108/actualiza_imagen.sh 2>&1'); header('Content-Type: application/json'); echo json_encode(['ok'=>true,'output'=>htmlspecialchars($o??'(sin salida)')]); exit; }
if ($action === 'update-ids')    { $o=shell_exec('sudo sh /home/pi/A108/actualizar_ids.sh 2>&1'); header('Content-Type: application/json'); echo json_encode(['ok'=>true,'output'=>htmlspecialchars($o??'(sin salida)')]); exit; }
if ($action === 'update-ysf')    { $o=shell_exec('sudo sh /home/pi/A108/actualizar_reflectores_ysf.sh 2>&1'); header('Content-Type: application/json'); echo json_encode(['ok'=>true,'output'=>htmlspecialchars($o??'(sin salida)')]); exit; }
...
if ($action === 'mmdvmysf-status') { header('Content-Type: application/json'); echo json_encode(['mmdvmysf'=>trim(shell_exec('systemctl is-active mmdvmysf 2>/dev/null'))]); exit; }
...
if ($action === 'mmdvmysf-logs')   { $n=intval($_GET['lines']??15); header('Content-Type: application/json'); echo json_encode(['mmdvmysf'=>htmlspecialchars(shell_exec("sudo journalctl -u mmdvmysf -n {$n} --no-pager --output=short 2>/dev/null")??'')]); exit; }
...
if ($action === 'install-display') { $o=shell_exec('sudo /home/pi/A108/instalar_displaydriver.sh 2>&1'); header('Content-Type: application/json'); echo json_encode(['ok'=>true,'output'=>htmlspecialchars($o??'')]); exit; }

if ($action === 'backup-configs') {
    $zipPath='/tmp/Copia_A108.zip';
    $files=['/home/pi/MMDVMHost/MMDVMHost.ini','/home/pi/MMDVMHost/MMDVMYSF.ini','/home/pi/Display-Driver/DisplayDriver.ini','/home/pi/YSFClients/YSFGateway/YSFGateway.ini','/home/pi/DMRGateway/DMRGateway.ini'];
    shell_exec("zip -j ".escapeshellarg($zipPath)." ".implode(' ',array_map('escapeshellarg',$files))." 2>/dev/null");
    if(file_exists($zipPath)){header('Content-Type: application/zip');header('Content-Disposition: attachment; filename="Copia_A108.zip"');header('Content-Length: '.filesize($zipPath));header('Pragma: no-cache');header('Expires: 0');readfile($zipPath);unlink($zipPath);}
    else{header('Content-Type: text/plain');echo 'Error: No se pudo crear el ZIP.';}
    exit;
}
...
if ($action === 'logs') {
    $n=intval($_GET['lines']??15);
    header('Content-Type: application/json');
    echo json_encode(['gateway'=>htmlspecialchars(shell_exec("sudo journalctl -u dmrgateway -n {$n} --no-pager --output=short 2>/dev/null")??''),'mmdvm'=>htmlspecialchars(shell_exec("sudo journalctl -u mmdvmhost -n {$n} --no-pager --output=short 2>/dev/null")??'')]);
    exit;
}
if ($action === 'ysf-logs') {
    $n=intval($_GET['lines']??15);
    $log=shell_exec("sudo journalctl -u ysfgateway -n {$n} --no-pager --output=short 2>/dev/null");
    if(empty(trim($log)))$log=shell_exec("tail -n {$n} /tmp/ysfgateway.log 2>/dev/null");
    if(empty(trim($log))){$lf=glob('/home/pi/YSFClients/YSFGateway/YSFGateway-*.log');if($lf){$log=shell_exec("tail -n {$n} ".escapeshellarg(end($lf))." 2>/dev/null");}}
...
if ($action === 'dstar-status') {
    ...
    header('Content-Type: application/json');
    echo json_encode(['gateway'=>trim(shell_exec('systemctl is-active dstargateway 2>/dev/null')),'mmdvm'=>trim(shell_exec('systemctl is-active mmdvmdstar 2>/dev/null')),'stopped'=>$stopped]);
    exit;
}
...
if ($action === 'dstar-logs')  { $n=intval($_GET['lines']??15); header('Content-Type: application/json'); echo json_encode(['gateway'=>htmlspecialchars(shell_exec("sudo journalctl -u dstargateway -n {$n} --no-pager --output=short 2>/dev/null")??''),'mmdvm'=>htmlspecialchars(shell_exec("sudo journalctl -u mmdvmdstar -n {$n} --no-pager --output=short 2>/dev/null")??'')]); exit; }
...
.btn-header.cyan{color:var(--cyan);border:1px solid var(--cyan);} .btn-header.cyan:hover{background:rgba(0,212,255,.1);}
.btn-header.amber{color:var(--amber);border:1px solid var(--amber);} .btn-header.amber:hover{background:rgba(255,179,0,.1);}
.btn-header.red{color:var(--red);border:1px solid var(--red);} .btn-header.red:hover{background:rgba(255,69,96,.15);}
...
.service-card-label.dmr{color:var(--amber);} .service-card-label.ysf{color:var(--violet);}
...
.toggle-label.on-dmr{color:var(--amber);} .toggle-label.on-ysf{color:var(--violet);}
...
.sw-track{position:absolute;inset:0;border-radius:2px;background:#1a2535;border:2px solid #999;transition:background .3s,border-color .3s;}
...
.ini-btn.edit{color:var(--amber);border-color:rgba(255,179,0,.3);} .ini-btn.edit:hover{border-color:var(--amber);background:rgba(255,179,0,.08);}
.ini-btn.view{color:var(--cyan);border-color:rgba(0,212,255,.3);} .ini-btn.view:hover{border-color:var(--cyan);background:rgba(0,212,255,.08);}
.ini-btn.edit.ysf{color:var(--violet);border-color:rgba(181,122,255,.3);} .ini-btn.edit.ysf:hover{border-color:var(--violet);background:rgba(181,122,255,.08);}
.ini-btn.view.ysf{color:#c9a0ff;border-color:rgba(181,122,255,.2);} .ini-btn.view.ysf:hover{border-color:var(--violet);background:rgba(181,122,255,.06);}
...
.nextion::before{top:.5rem;left:.7rem;} .nextion::after{bottom:.5rem;right:.7rem;}
...
.nextion-ysf::before{top:.5rem;left:.7rem;} .nextion-ysf::after{bottom:.5rem;right:.7rem;}
...
.nx-topbar .nx-mode{color:var(--cyan);opacity:.7;} .nx-topbar.ysf-bar .nx-mode{color:var(--violet);opacity:.8;}
...
.nx-info-val.cyan{color:var(--cyan);} .nx-info-val.amber{color:var(--amber);} .nx-info-val.green{color:var(--green);}
...
.lh-body::-webkit-scrollbar{width:3px;} .lh-body::-webkit-scrollbar-thumb{background:var(--border);}
...
.lh-row:last-child{border-bottom:none;} .lh-row:hover{background:rgba(0,212,255,.04);} .lh-row.lh-active{background:rgba(0,255,159,.06);}
...
.lh-src.rf{color:var(--green);} .lh-src.net{color:var(--cyan);}
...
.lh-row-ysf:last-child{border-bottom:none;} .lh-row-ysf:hover{background:rgba(181,122,255,.04);} .lh-row-ysf.lh-active{background:rgba(181,122,255,.08);}
...
.lh-src-ysf{font-family:var(--font-mono);font-size:.6rem;}
.lh-src-ysf.rf{color:var(--green);} .lh-src-ysf.net{color:var(--cyan);}
#ysfLastHeardPanel{grid-column:2;} #ysfDisplayPanel{grid-column:2;}
...
.log-panel-header .svc-name.gw{color:var(--amber);} .log-panel-header .svc-name.ysf{color:var(--violet);}
...
.log-output::-webkit-scrollbar{width:4px;} .log-output::-webkit-scrollbar-thumb{background:var(--border);border-radius:2px;}
.ln-info{color:#7a9ab5;} .ln-warn{color:var(--amber);} .ln-err{color:var(--red);} .ln-ok{color:var(--green-dim,#00cc7a);}
...
.update-console .ok{color:var(--green);} .update-console .err{color:var(--red);} .update-console .inf{color:#7a9ab5;}
/* Terminal */
.term-modal{display:none;position:fixed;inset:0;background:rgba(0,0,0,.88);z-index:9600;align-items:center;justify-content:center;}
.term-modal.open{display:flex;}
.term-box{background:#0d1117;border:1px solid #00ff9f;border-radius:8px;width:820px;max-width:96vw;display:flex;flex-direction:column;box-shadow:0 0 30px rgba(0,255,159,.12);}
.term-bar{background:#161b22;border-bottom:1px solid #21262d;padding:.45rem 1rem;display:flex;align-items:center;gap:.5rem;border-radius:8px 8px 0 0;}
.term-dot{width:12px;height:12px;border-radius:50%;display:inline-block;}
.term-dot-r{background:#ff5f57;cursor:pointer;} .term-dot-y{background:#febc2e;} .term-dot-g{background:#28c840;}
.term-ttl{font-family:var(--font-mono);font-size:.72rem;color:#8b949e;flex:1;text-align:center;}
.term-out{font-family:var(--font-mono);font-size:.78rem;line-height:1.6;color:#c9d1d9;padding:.8rem 1rem;overflow-y:auto;height:380px;white-space:pre-wrap;word-break:break-all;background:#0d1117;}
.term-out::-webkit-scrollbar{width:4px;} .term-out::-webkit-scrollbar-thumb{background:#30363d;}
.term-row{display:flex;align-items:center;gap:.5rem;padding:.45rem 1rem;border-top:1px solid #21262d;background:#161b22;border-radius:0 0 8px 8px;}
.term-pr{font-family:var(--font-mono);font-size:.78rem;color:#00ff9f;white-space:nowrap;}
.term-inp{flex:1;background:transparent;border:none;outline:none;font-family:var(--font-mono);font-size:.78rem;color:#c9d1d9;caret-color:#00ff9f;}
.tc{color:#e6edf3;} .to{color:#8b949e;} .te{color:#f85149;}
</style>
</head>
<body>
...
<button class="btn-header cyan" onclick="abrirTerminal()">⌨ Terminal</button>
</div>
</header>
...
<!-- Modal Terminal -->
<div id="termModal" class="term-modal" onclick="if(event.target===this)cerrarTerminal()">
<div class="term-box">
  <div class="term-bar">
    <span class="td td-r" onclick="cerrarTerminal()" title="Cerrar"></span>
    <span class="td td-y"></span>
    <span class="td td-g"></span>
    <span class="term-ttl" id="termTtl">pi@pi:~ $</span>
  </div>
  <div class="term-out" id="termOut">EA3EIZ · Terminal · Panel Sistemas Digitales
─────────────────────────────────────────────
</div>
  <div class="term-row">
    <span class="term-pr" id="termPr">pi@pi:~$</span>
    <input id="termInp" class="term-inp" autocomplete="off" spellcheck="false" placeholder="escribe un comando…">
  </div>
</div>
</div>
...
function getFlagByCall(callsign){if(!callsign)return'';const cs=callsign.toUpperCase().trim();const prefixes=[{re:/^EA[0-9]|EB|EC|ED|EE|EF|EG|EH/,flag:'🇪🇸'},{re:/^CT|CU|CV|CQ/,flag:'🇵🇹'},{re:/^F[A-Z]|FM|FO/,flag:'🇫🇷'},{re:/^I[0-9]|IK|IW|IZ/,flag:'🇮🇹'},{re:/^G[0-9]|M[0-9]|2E[0-9]|GB|MJ|MU/,flag:'🇬🇧'},{re:/^D[ALM]|DA|DB|DC|DD|DE|DF|DG|DH|DI|DJ|DK|DL|DM|DN|DO|DP|DQ|DR/,flag:'🇩🇪'},{re:/^K[0-9]|W[0-9]|N[0-9]|AA|AB|AC|AD|AE|AF/,flag:'🇺🇸'},{re:/^VE[0-9]|VA[0-9]/,flag:'🇨🇦'},{re:/^PY[0-9]|PU|PV|PW|PX/,flag:'🇧🇷'},{re:/^LU[0-9]|LV|LW/,flag:'🇦🇷'},{re:/^JA[0-9]|JE|JF|JG|JH|JI|JJ|JK|JL|JM|JN|JO|JP|JR|JS/,flag:'🇯🇵'},{re:/^VK[0-9]/,flag:'🇦🇺'},{re:/^ZS[0-9]/,flag:'🇿🇦'},{re:/^OH[0-9]|OG|OI|OJ/,flag:'🇫🇮'},{re:/^PA[0-9]|PB|PC|PD|PE|PF|PG|PH/,flag:'🇳🇱'},{re:/^HB[0-9]|HB9/,flag:'🇨🇭'},{re:/^OE[0-9]/,flag:'🇦🇹'},{re:/^SP[0-9]|SQ|SR/,flag:'🇵🇱'},{re:/^UA[0-9]|UB|UC|UD|UE|UF|UG|UH|UI|UJ|UK|UL|UM|UN|UO|UP|UQ|UR|US|UT|UU|UV|UW|UX|UY|UZ/,flag:'🇷🇺'},{re:/^SV[0-9]|SW|SX|SY|SZ/,flag:'🇬🇷'},{re:/^LY[0-9]|LZ/,flag:'🇱🇹'},{re:/^9A[0-9]/,flag:'🇭🇷'}];for(const p of prefixes){if(p.re.test(cs))return p.flag;}return'🌐';}
...
function setDSTARToggle(on){const chk=document.getElementById('chkDSTAR'),lbl=document.getElementById('dstarToggleLabel'),sta=document.getElementById('dstarToggleStatus');chk.checked=on;lbl.style.color=on?'#00e5ff':'';sta.className='toggle-status'+(on?' on':'');sta.textContent=on?'ON':'OFF';document.getElementById('dstarRefreshBadge').style.display=on?'flex':'none';document.getElementById('dstarLogPanels').style.display=on?'contents':'none';}
...
/* ── Terminal ── */
var termHist=[],termHidx=-1,termCwd='/home/pi';
function abrirTerminal(){document.getElementById('termModal').classList.add('open');setTimeout(function(){document.getElementById('termInp').focus();},80);}
function cerrarTerminal(){document.getElementById('termModal').classList.remove('open');}
function termPrStr(){return 'pi@pi:'+termCwd.replace('/home/pi','~')+'$';}
function termApp(html){var o=document.getElementById('termOut');o.innerHTML+=html+'\n';o.scrollTop=o.scrollHeight;}
function termEsc(s){return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');}
document.getElementById('termInp').addEventListener('keydown',async function(e){
    if(e.key==='Escape'){cerrarTerminal();return;}
    if(e.key==='ArrowUp'){e.preventDefault();if(termHidx<termHist.length-1)this.value=termHist[++termHidx]||'';return;}
    if(e.key==='ArrowDown'){e.preventDefault();termHidx>0?this.value=termHist[--termHidx]:(termHidx=-1,this.value='');return;}
    if(e.key!=='Enter')return;
    var cmd=this.value.trim();if(!cmd)return;
    termHist.unshift(cmd);termHidx=-1;this.value='';
    termApp('<span class="tc">'+termEsc(termPrStr())+' '+termEsc(cmd)+'</span>');
    if(/^\s*cd(\s|$)/.test(cmd)){var t=(cmd.replace(/^\s*cd\s*/,'').trim()||'~');termCwd=(t==='~')?'/home/pi':t;document.getElementById('termPr').textContent=termPrStr();document.getElementById('termTtl').textContent=termPrStr()+' $';return;}
    try{var resp=await fetch('?action=terminal',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'cmd='+encodeURIComponent('cd '+termCwd+' && '+cmd)});var dat=await resp.json();if(dat.output)termApp('<span class="to">'+dat.output+'</span>');}
    catch(err){termApp('<span class="te">Error: '+termEsc(err.message)+'</span>');}
});
